...
secure_t * ecies_encrypt(char *key, unsigned char *data, size_t length) {
	...
	EVP_CIPHER_CTX cipher = {0};
	unsigned int mac_length = 0;
	EC_KEY *user = NULL,*ephemeral = NULL;
	size_t envelope_length = 0, block_length = 0, key_length = 0;
	unsigned char envelope_key[SHA512_DIGEST_LENGTH] = {0}, iv[EVP_MAX_IV_LENGTH] = {0}, block[EVP_MAX_BLOCK_LENGTH] = {0};
...
unsigned char * ecies_decrypt(char *key, secure_t *cryptex, size_t *length) {
	...
	unsigned char envelope_key[SHA512_DIGEST_LENGTH] = {0}, iv[EVP_MAX_IV_LENGTH] = {0}, md[EVP_MAX_MD_SIZE] = {0}, *block, *output;
	...
	// At the moment we are generating the hash using encrypted data. At some point we may want to validate the original text instead.
	if (HMAC_Init_ex(&hmac, envelope_key + key_length, key_length, ECIES_HASHER, NULL) != 1 || HMAC_Update(&hmac, secure_body_data(cryptex),
