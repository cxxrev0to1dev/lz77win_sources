...
STDMETHODIMP C7ZipArchiveOpenCallback::CryptoGetTextPassword(BSTR *password)
{
    if (!PasswordIsDefined) {
        //FIXME
      if (ask_open_password_)
      {
        ask_open_password_->AskPasswordUI();
        Password = ask_open_password_->GetPassword();
        PasswordIsDefined = true;
#ifdef _WIN32
        return StringToBstr(ask_open_password_->GetPassword(), password);
#else
