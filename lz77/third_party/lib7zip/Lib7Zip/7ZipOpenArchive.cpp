                                   ...
                                   bool fCheckFileTypeBySignature)
{
  ...
  if (pOpenCallBack)
  {
    pOpenCallBack->ask_open_password_ = pLibrary->GetAskOpenArchivePassword();
  }
	CMyComPtr<IArchiveOpenCallback> openCallBack(pOpenCallBack);

	do {
    //MOD:ADD:"if (archive)" read pe CreateInArchive Recursive call
    if (archive) break;
...
/*
MOD:DEL:dengtao read pe data error
